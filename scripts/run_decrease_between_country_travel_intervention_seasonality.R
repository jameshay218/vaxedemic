...
seedCountries <- "China"
  ...
  outputDir <- "decrease_between_country_travel/"
  
  output_prefix <- "intervention_seasonality"
  ...
  n_runs <- 100
  intervention_params <- list(tintervene = 35, prop_travel = 0.5)
  ...
  seasonality_params <- list(tdelay = -90, # (in days) Shifts the seasonality function - changing this effectively changes the seed time.
  ...
  # latent period same as Donnelly et al. (2003)
  life_history_params <- list(R0=2/1.3, TR=3.6, LP = 6.4, case_fatality_ratio = rep(1e-3,2))
  ...
  seed_params <- list(Countries = seedCountries, # where to seed
                      Sizes = c(100), # how many to seed in each country
  ...
  if(run_fixed) {
    ...
    ################################################################################
    run_func <- "run_fixed_params_and_postprocess_intervention"
    if(cluster) {
      # submit to cluster
