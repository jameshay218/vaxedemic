...
# library(plyr)
# devtools::load_all()
load_GAR <- function(run_name) {
  filename <- paste0("outputs_", run_name, "/GAR.rds")
  if(!file.exists(filename)) {
    compile_GAR(run_name)
  } else {
    readRDS(filename)
  }
}
...
compile_GAR <- function(run_name) {
  if(run_name == "top_n_countries") {
    postprocess <- function(n_countries) {
      GAR <- paste0("outputs_top_n_countries/by_pop_size_coverage_data_",
    ...
    postprocess <- function(alpha) {
      GAR <- paste0("outputs_fn_pop_size/fn_pop_size_coverage_data_",
    ...
    GAR <- readRDS("outputs_incidence/pd7_vacbyinc_incidence.rds") %>%
      vnapply(., sum)
...
ggsave("all_GAR.pdf", g, width = 20, height = 35, units = "cm")
