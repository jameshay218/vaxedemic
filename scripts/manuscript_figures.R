read_median_global_deaths <- function(strategy, production_delay, stockpile_size, free_param) {
...
read_global_deaths <- function(strategy, production_delay, stockpile_size, free_param) {
  dir_name <- paste0("outputs/deaths_only/pd", production_delay, strategy, 
                     "_stockpile", num2str(stockpile_size), "/")
...
bootstrap_deaths_averted <- function(strategies, production_delays, stockpile_sizes, free_params) {
  ...
  global_deaths <- Map(read_global_deaths, strategies, production_delays, stockpile_sizes, free_params) %>%
...
bootstrap_median_deaths <- function(strategy, production_delay, stockpile_size, free_param) {
  
  global_deaths <- read_global_deaths(strategy, production_delay, stockpile_size, free_param)
...
bootstrap_median_deaths_wrapper <- function(strategy, production_delay, stockpile_size = 0, free_param = 0) {
  production_delay <- as.numeric(production_delay)
  stockpile_size <- as.numeric(stockpile_size)
  free_param <- as.numeric(free_param)
  bootstrap_sol <- bootstrap_median_deaths(strategy, production_delay, stockpile_size, free_param)
  quantile(bootstrap_sol$t, probs = c(.025, .5, .975))
}

bootstrap_deaths_averted_wrapper <- function(strategy, production_delay, stockpile_size = 0, free_param = 0) {
  production_delay <- as.numeric(production_delay)
  stockpile_size <- as.numeric(stockpile_size)
  free_param <- as.numeric(free_param)
  ...
  bootstrap_sol <- bootstrap_deaths_averted(strategies, production_delays, stockpile_sizes, free_params)
  quantile(bootstrap_sol$t, probs = c(.025, .5, .975))
}
...
make_Fig3 <- function(save_output, bootstrap = FALSE) {
  ...
  if(bootstrap) {
    plot1 <- plot1 + geom_errorbar(aes(ymin = lower_deaths_averted, ymax = lower_deaths_averted))
    plot2 <- plot2 + geom_errorbar(aes(ymin = lower_deaths_averted, ymax = lower_deaths_averted), position = "dodge")
  }
